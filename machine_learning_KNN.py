...
def main(argv):
	...
	if len(argv) > 4:
		...
		writeToCSV.writeToCSV(rows, columns, headers, types, A, "machine_learning_knn.csv")
	else:
		print "Now converting to CSV file"
		B = dtest.get_all_data()
		B[:,-1] = ctestcats
		headers = dtest.get_headers()
		types = dtest.get_raw_types()
		writeToCSV.writeToCSV(rows, columns, headers, types, B, "machine_learning_knn.csv")
